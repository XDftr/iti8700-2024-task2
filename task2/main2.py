...
class ChatAI:
    ...
    def ask_chat(self, data, message, expected):
        ...
        if self.enable_gpt3 and self.enable_gpt4:
    ...
    def read_input_data(self):
        ...
        with open(file_path, "r") as file:
            for line in file.readlines():
                ...
                questions = json_object["questions.txt"]
                parsed_questions = []
                ...
                self.input_data.append({
                    "data": text,
                    "questions.txt": parsed_questions
                })
    ...
    def ask_questions(self):
        for data in self.input_data:
            ...
            for q in data["questions.txt"]:
                ...
                if self.enable_gpt3:
                    response["gpt3_response"] = gpt3_response

                if self.enable_gpt4:
                    response["gpt4_response"] = gpt4_response

                if text in self.output_data.keys():
                    self.output_data[text].append(response)
                else:
                    self.output_data[text] = [response]

    def save_responses(self):
        file_path = "output3.txt"
        with open(file_path, "w") as file:
            if self.enable_gpt4 and self.enable_gpt3:
                file.write(
                    f"Total: {self.total}, "
                    f"GPT3 True: {self.gpt3_true} ({round((self.gpt3_true / self.total * 100), 2)}%), "
                    f"GPT4 True: {self.gpt4_true} ({round((self.gpt4_true / self.total * 100), 2)}%), "
                    f"Both wrong: "
                    f"{self.gpt4_and_3_wrong} ({round((self.gpt4_and_3_wrong / self.total * 100), 2)})\n\n")
            elif self.enable_gpt4:
                file.write(f"Total: {self.total}, "
                           f"GPT4 True: {self.gpt4_true} ({round((self.gpt4_true / self.total * 100), 2)}%)\n\n")
            elif self.enable_gpt3:
                file.write(f"Total: {self.total}, "
                           f"GPT3 True: {self.gpt3_true} ({round((self.gpt3_true / self.total * 100), 2)}%)\n\n")

            for response in self.output_data.keys():
                file.write("********************\n")
                file.write(response + "\n\n")
                for question in self.output_data[response]:
                    q = question["question"]
                    g3 = question.get("gpt3_response", "")
                    g4 = question.get("gpt4_response", "")
                    e = question["expected_response"]
                    if self.enable_gpt3 and self.enable_gpt4:
    ...
    def run(self):
        if not self.enable_gpt4 and not self.enable_gpt3:
            raise RuntimeError("At least 1 gpt must be enabled!")
        self.ask_questions()
        self.save_responses()
